...
public class ConnectionFactory {
	...
	public ConnectionFactory(String[] endpoint,String url){
		this.API = url;													 //url包括整个api
		for(int i = 0; i<endpoint.length;i++){
			String[] points = endpoint[i].split(",");
			for(int f = 0; f<points.length;f++){
				fields.put(points[f].split(":")[0], points[f].split(":")[1]);   // fields[0] 是key  fields[1] 是value
			}
		}
	}
		...
		if(!this.getEndpoints().equalsIgnoreCase("") && !this.getEndpoints().isEmpty()){
			...
			try {
				for(Map.Entry<String,String> entry: fields.entrySet()){
					vars = entry.getKey();
					vals = entry.getValue();
					data +=("&"+vars+"="+vals); // 把key value 变成 &key1=value1&key2=value2.....
				}
				if(data.startsWith("&")){ 
					data = data.replaceFirst("&", "");   // 把前边有&去掉
						}
